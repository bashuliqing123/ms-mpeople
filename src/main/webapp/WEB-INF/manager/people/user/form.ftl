<@ms.html5>
    ...
    <@ms.panel>
    	<@ms.form name="peopleUserForm" isvalidation=true>
			...
			<@ms.text name="peopleName" label="用户名" placeholder="请输入用户名"  title="" size="5" width="240"  value="${(peopleUserEntity.peopleName)?default('')}" validation={"required":"true","data-bv-stringlength":"true","data-bv-regexp":"true","data-bv-regexp-regexp":'^[^[!@#$%^&*()_+-/~?！@#￥%…&*（）——+—？》《：“‘’]+$',"data-bv-stringLength-min":"6" ,"data-bv-stringlength-max":"30","data-bv-regexp-message":"用户名长度不能超过30个字符且不能包含特殊字符","data-bv-stringLength-message":"用户名长度为6到30个字符", "data-bv-notempty-message":"必填项目"}/>
			<@ms.password name="peoplePassword" label="密码"   title="" size="5" width="240"  validation={"data-bv-stringlength":"true","data-bv-stringlength-max":"20","data-bv-stringLength-min":"6" ,"data-bv-stringlength-message":"密码长度不能超过20个字符","data-bv-regexp":"true","data-bv-regexp-regexp":'^[A-Za-z0-9_]+$',"data-bv-regexp-message":"密码只能由英文字母，数字，下划线组成!"}/>
			...
			<@ms.formRow label="头像" width="400">
					<@ms.uploadImg path="people" inputName="puIcon" size="1" filetype="" msg="提示:文章缩略图,支持jpg格式"  maxSize="2" imgs="${(peopleUser.peopleUserIcon)?default('')}"  />
			</@ms.formRow>
