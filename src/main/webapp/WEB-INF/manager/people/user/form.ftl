<@ms.html5>
    ...
    <@ms.panel>
    	<@ms.form name="peopleUserForm" isvalidation=true>
			...
			<@ms.text name="peopleName" label="用户名" placeholder="请输入用户名"  title="" size="5" width="240"  value="${(peopleUserEntity.peopleName)?default('')}" validation={"data-bv-stringlength":"true","data-bv-regexp":"true","data-bv-regexp-regexp":'^[^[!@#$%^&*()_+-/~?！@#￥%…&*（）——+—？》《：“‘’]+$',"data-bv-stringlength-max":"12","data-bv-regexp-message":"用户名长度不能超过12个字符且不能包含特殊字符"}/>
			<@ms.password name="peoplePassword" label="密码"   title="" size="5" width="240"  validation={"data-bv-stringlength":"true","data-bv-stringlength-max":"20","data-bv-stringlength-message":"密码长度不能超过20个字符","data-bv-regexp":"true","data-bv-regexp-regexp":'^[A-Za-z0-9_]+$',"data-bv-regexp-message":"密码只能由英文字母，数字，下划线组成!"}/>
			...
			<@ms.formRow label="头像" width="400">
					<@ms.uploadImg path="upload/people/${appId?default(0)}/" inputName="puIcon" size="1" filetype="" msg="提示:文章缩略图,支持jpg格式"  maxSize="2" imgs="${(peopleUser.peopleUserIcon)?default('')}"  />
			</@ms.formRow>
			<@ms.text label="用户身份证" name="puCard" value="${(peopleUserEntity.puCard)?default('')}"  width="240px;" placeholder="请输入身份证" validation={"data-bv-stringlength":"true","data-bv-stringlength-max":"18","data-bv-regexp":"true","data-bv-regexp-regexp":'^[X0-9]+$',"data-bv-regexp-message":"身份证输入不合法"}/>
